...
## RUNNING NOW
- **Tilt sweep** (`probe_tilt_sweep.py`, 5 directions × 800 games) — the one sound
  free live-exploitability probe (no battery reference). Tests if any crude direction
  beats champion +0.746 vs the field. Verdict pending.

## NEXT IN QUEUE (autonomous, free, cheapest-first)
1. **Postflop probe** — biggest UNTESTED model dimension (all work so far is preflop).
   No data gate. Design: where, if anywhere, does the champion leak EV postflop vs the
   field. ← runs after the tilt sweep.
2. Strong-opponent robustness check (only beat a weak scripted field so far).
