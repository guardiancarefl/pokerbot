...
# --------------------------------------------------------------------------
# Anchored commit reconciliation (CR — 2026-06-13 session-5 build; the
# MIRROR family of P2's displacement class, pre-registered in
# evals/p2_session5_validation_20260613/REPORT.txt). Three live casualties
# on 2026-06-12 (seq 780 8c8d, seq 1086 AcAh, seq 1635 5hTs) proved the
# class: the SCRAPER frame is RIGHT (anchor-conserving, confirmed by hand
# outcomes 3/3) and the RECONSTRUCTION under-counts a non-hero seat's
# committed chips — swept folded blinds (780), limp-then-fold (1086),
# partial commits — because a folded seat's bet field renders null and the
# preflop derivation reads per-seat commits off `frame.bet`. The fix is
# REPLAY-SIDE: rebuild the believed action sequence from per-seat IMPLIED
# commits against the clean P1 hand-start anchor
# (pre_hand[i] − frame.stack[i] − ante = seat i's blind+voluntary commit
# this hand), re-derive a legal OpenSpiel sequence reproducing those
# commits via the existing derive_action_sequence, and re-validate the
# UNCHANGED scraper frame through the replay + invariant gate. The frame is
# never patched (that is P2's direction, proven backwards for this class).
# Refuse on any ambiguity: non-conserving frames, no clean REGULAR anchor
# (which doubles as the positive blind-structure evidence — is_hand_start
# fires only when BOTH blinds were seen posted, so dead-SB hands like the
# seq-276 fixture can never serve one), implied commits below visible
# bets, or live seats whose implied commit disagrees with their bet.
# --------------------------------------------------------------------------


def classify_commit_reconciliation_deltas(deltas) -> str:
    """Class-membership test for the commit-reconciliation family.

    Returns:
      "candidate"     — every invariant delta is a pot / stack[seatN] /
                        bet[seatN] field (any seat count: the family's
                        exhibits span 1-seat and 2-seat under-counts).
      "not_signature" — any other delta type appears (current_player,
                        street_idx, cards, scraper_self_consistency,
                        legal_actions, ...): corruption beyond a commit
                        under-count. The caller must leave the frame's
                        drop byte-identical (no annotation), mirroring
                        P2's not_signature contract.
    """
    if not deltas:
        return "not_signature"
    for name, _scraper_val, _recon_val in deltas:
        n = str(name)
        if n == "pot":
            continue
        if _DELTA_STACK_RE.match(n) or _DELTA_BET_RE.match(n):
            continue
        return "not_signature"
    return "candidate"


def implied_commits_from_anchor(
    frame: ScraperFrame, pre_hand: tuple[int, ...],
) -> dict[int, int]:
    """Per-seat implied blind+voluntary commit this hand, from the anchored
    pre-hand stacks: commit[i] = pre_hand[i] − frame.stack[i] − ante for
    every seat at hand start (pre_hand[i] > 0). Pure arithmetic — callers
    own all validation (see build_commit_reconciliation_shadow)."""
    ante = int(frame.blinds.ante)
    return {
        i: int(pre_hand[i]) - int(frame.stack[i]) - ante
        for i in range(NUM_SEATS) if int(pre_hand[i]) > 0
    }


def build_commit_reconciliation_shadow(
    frame: ScraperFrame, pre_hand: tuple[int, ...],
) -> tuple[ScraperFrame | None, dict[int, int] | None, str]:
    """Validate the commit-reconciliation preconditions and build the
    DERIVATION SHADOW frame: a copy of `frame` whose FOLDED seats' bet
    fields are restored to their anchor-implied commits.

    The shadow exists only to feed derive_action_sequence /
    replay_to_decision — restoring a folded blind's post (seq 780) lets
    Predicate 1 re-detect the SB/BB posters the fold swept away, and a
    restored limp (seq 1086) re-enters the delayed-fold emission path.
    The TRUE frame is what the caller re-validates and decides on; the
    shadow never reaches the invariant, the tracker, or the model.

    Gates — ALL must hold, else (None, None, why):
      1. preflop frame (board empty). The proven class is the preflop
         sweep family; postflop derivation already reads folded seats'
         commits from stack arithmetic, so a postflop invariant_fail in
         these fields is NOT this class.
      2. every currently-alive seat is at hand start per the anchor
         (anchor/frame seat-composition agreement).
      3. chip conservation, strict convention: Σ frame.stack[at-start]
         + frame.pot_total == Σ pre_hand. (The caller's frame is already
         UI-lag pot-corrected by SessionTracker where applicable.)
      4. every implied commit is >= 0 and >= the seat's visible bet.
      5. hero's implied commit EQUALS hero's visible bet (hero's state is
         never swept — they are to act; any disagreement is not this
         class).
      6. every alive NON-folded seat's implied commit EQUALS its visible
         bet (a live seat's chips this street are all on display preflop;
         disagreement means mid-collection capture or another class).
         Seats alive at start but reading alive=False (busted-mid-hand /
         all-in sweeps) are exempt: the existing busted-mid-hand emission
         in derive_action_sequence owns their chips.

    Returns (shadow_frame, restored, "ok") where restored maps seat ->
    implied commit for each folded seat whose bet was restored (possibly
    empty: the rebuild can still matter via exact-emission ante
    bookkeeping, live exhibit seq 1635).
    """
    if frame.board:
        return None, None, (
            "not a preflop frame (the proven commit-sweep class is "
            "preflop-only; postflop derivation already implies folded "
            "commits from stack arithmetic)")
    ante = int(frame.blinds.ante)
    seats_at_start = [
        i for i in range(NUM_SEATS) if int(pre_hand[i]) > 0
    ]
    for i in range(NUM_SEATS):
        if frame.alive[i] and i not in seats_at_start:
            return None, None, (
                f"seat{i + 1} is alive but absent from the anchor "
                f"(pre_hand=0) — anchor does not describe this hand")
    stack_sum = sum(int(frame.stack[i]) for i in seats_at_start)
    expected_total = sum(int(pre_hand[i]) for i in seats_at_start)
    if stack_sum + int(frame.pot_total) != expected_total:
        return None, None, (
            f"frame does not conserve chips against the anchor "
            f"({stack_sum} + {frame.pot_total} != {expected_total})")
    commits = implied_commits_from_anchor(frame, pre_hand)
    ...
    for i in seats_at_start:
        ...
        if c < 0:
            return None, None, (
                f"seat{i + 1} implied commit {c} negative "
                f"(anchor/frame disagree)")
        if c < b:
            return None, None, (
                f"seat{i + 1} implied commit {c} below visible bet {b}")
        if i == frame.hero_seat:
            if c != b:
                return None, None, (
                    f"hero implied commit {c} != visible bet {b} "
                    f"(hero state is never swept — not this class)")
            continue
        if not frame.alive[i]:
            # Busted-mid-hand / fully-swept seat: the existing
            # busted-mid-hand emission owns their chips.
            continue
        if frame.folded[i]:
            if c > b:
                restored[i] = c
        ...
        if c != b:
            return None, None, (
                f"live seat{i + 1} implied commit {c} != visible bet {b} "
                f"(mid-collection capture or another class)")

    if not restored:
        # Nothing to restore — the shadow IS the frame. The rebuild can
        # still recover via exact-emission ante bookkeeping (seq 1635).
        return frame, {}, "ok"

    bet = list(frame.bet)
    for i, c in restored.items():
        bet[i] = int(c)
    # Recompute parse_frame's derived fields so the shadow is
    # indistinguishable from a cleanly parsed frame downstream — same
    # contract as _rebuild_frame_with_stack / _rebuild_frame_with_bet_closure.
    alive = tuple(
        (not frame.empty[i]) and (int(frame.stack[i]) > 0 or bet[i] > 0)
        for i in range(NUM_SEATS)
    )
    if alive[frame.hero_seat]:
        max_opp_bet = max(
    ...
    shadow = dataclasses.replace(
        frame, bet=tuple(bet), alive=alive,
        hero_facing_bet=hero_facing_bet)
    return shadow, restored, "ok"
